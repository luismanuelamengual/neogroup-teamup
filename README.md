...
- **Late registration** (see below): the organizer can still register a competitor after the tournament started, but only where the structure already has room for one.
...
The **Administrar** page stays available once a tournament is ONGOING, but it narrows down to a single action: **registering a competitor**. Categories cannot be added or removed, competitors cannot be moved, unregistered or re-seeded — the structure *is* the tournament at that point, and none of those can be done without rewriting matches that are already being played. The rules are pure and live in `app/(protected)/(tournaments)/utils/lateRegistration.ts`, shared by the admin page (to decide what to offer) and by the server (to decide what to accept), so the two can never disagree.
