...
- **Sedes** (`app/(protected)/(sites)`, table `sites`: `id`, `organizationId`, `name`, `data`) — the venues a tournament can be played at. `data` is a JSON document (migration `019-sites-data`) holding the venue's **courts setup** — how many courts it has, how they are named, and the duration it was last planned with; see "Planning and publishing the schedule". They replaced the free-text `tournaments.location` column, where the same club ended up spelled in as many ways as organizers there were; migration `008-sites` creates a site per distinct location and repoints every tournament at it through `tournaments.siteId`.
...
- **Leagues**: a single round-robin lane over the whole category. Two optional settings shape it — `allowUnorderedResults` (the whole fixture is materialised up front and any match accepts a result at any time, with `maxRounds` re-read as matches per competitor) and `doubleRound` (**ida y vuelta**: the round robin is played twice, the return leg replaying the first from the start with the sides inverted, so every pair meets once at home and once away). `doubleRound` simply doubles the natural number of rounds — `maxRounds` still caps that doubled schedule, and the two settings combine freely. Interclubes has its own home-and-away variant derived from the entry count (see below) and does not use this setting.
- **Groups + playoff**: the organizer sets the target group size and how many of each group qualify. Optionally a **minimum number of competitors reaching the knockout** (`minPlayoffQualifiers`) takes precedence over the per-group figure, but only upwards: `resolveGroupQualifiers` raises the cut-off **evenly across every group** until the total reaches the minimum, or until the field runs out (a huge value therefore means "everybody advances"). Because the level is shared, uneven groups may overshoot it — it is a floor, not a target. Two groups of 4 with 2 qualifiers and a minimum of 6 send the top 3 of each, not "the top 2 plus the two best runners-up". A groups+playoff **always** ends in a bracket, a single group included (see `tests/FINDINGS.md`, BUG #4).
- **Knockout seeding after a groups phase**: never random. Each group is ranked by its own points (`rankGroup`), the qualifiers are flattened **tier by tier** — all group winners, then all runners-up, ... — and within a tier they are ordered by group points, ties broken by the pre-tournament ranking seed and finally the competitor id, so the result is deterministic. The tier is the primary key because raw points are not comparable across groups of different sizes. `repairSameGroupPairings` then swaps first-round rivals so nobody replays a group opponent straight away, moving only the slots that face the top seeds. The ranking seeds themselves are used just once, at the start, to snake-seed the groups.
...
- The organizer starts the tournament manually (`POST /api/startTournament`), closes each round once all results are loaded and opens the next one; pairings are computed automatically based on the tournament type. `GET /api/processTournaments` (a Vercel Cron job, see `vercel.json`, secured by `CRON_SECRET`) only auto-finishes ONGOING tournaments once every round and match is complete — it no longer auto-starts STAND_BY tournaments; starting is manual only.
...
- **Late registration and late removal** (see below): after the tournament started the organizer can still register a competitor where the structure already has room, and unregister or move one to another category when they can come out without leaving a trace.
...
## Late registration (registering into a running tournament)

The **Administrar** page stays available once a tournament is ONGOING, but it narrows down to the entrant list: **registering, unregistering and moving a competitor**. Categories cannot be added or removed and nobody can be re-seeded — the structure *is* the tournament at that point, and neither can be done without rewriting matches that are already being played. The rules are pure and live in `app/(protected)/(tournaments)/utils/lateRegistration.ts` (arrivals) and `app/(protected)/(tournaments)/utils/lateRemoval.ts` (departures), shared by the admin page (to decide what to offer) and by the server (to decide what to accept), so the two can never disagree.

A late entrant is accepted **only where the structure already has a hole shaped like them**, never by growing or re-seeding anything. There are exactly two:

- **A knockout bye** (`PLAYOFF`). An entrant count that is not a power of two leaves top seeds unopposed in the first round, stored as a `WALKOVER` with no away side. Filling that side turns it into a real match and changes nothing else — same bracket size, same shape, same other pairings. Offered only while the bye's occupant has not moved on, i.e. the match it feeds is still `PENDING` (and, with a consolation bracket, the consolation slot that mirrors it too). Filling it re-runs the winner propagation, so the slot the occupant had been projected into goes back to "a definir".
- **A round-robin lane with room**: a group of a groups+playoff still in its group phase, or the single lane of a league. What counts as "room" depends on whether that lane's rounds are a **schedule** or just a **layout** (`allowUnorderedResults`):
  - **Ordered** (the default) — the round *is* the schedule, so a fixture may never change rounds. Only an **odd** round robin qualifies: the circle method pads it with a null slot that rests one competitor per round, and appending the entrant to the lane's id array puts them exactly there. That provably preserves **every pairing the lane already had** (each pair that did not involve the null is unchanged; the ones that did are now the entrant's) and its **number of rounds** — an odd round robin of size *k* needs as many as the even one of size *k+1* it becomes. Already-materialised rounds are reconciled rather than rebuilt: existing fixtures are kept and only renumbered, and the entrant's are inserted **in the round they belong to**, including rounds the rest of the lane already finished.
  - **Unordered** — the whole round robin exists up front and any match can be loaded at any time, so a round is a display grouping with no meaning of its own. **Parity stops mattering**: the complete round robin of a set is always contained in the complete round robin of that set plus one, so *no existing fixture changes at all*. The entrant's matches are appended into the lowest-numbered rounds where both sides are still free (a round is added at the end when none has space), so nobody is ever booked twice in the same round. On an odd lane this reproduces exactly what the circle method would have produced, because the free slots *are* the rest slots. A per-competitor quota (`maxRounds` read as `matchesPerCompetitor`) keeps working untouched: the entrant's surplus fixtures are voided by `syncUnorderedVoids` like anyone else's.

In every case the lane must still have something left to play — a round robin whose every fixture is resolved or voided is a finished competition, not one with room. The exception is a category that **materialised nothing at all**: starting a tournament skips every category left with a single competitor (one entrant cannot play anybody), so it never reached any phase. Registering a second competitor is what finally starts it, and it is the safest case there is — there is no fixture to preserve.

Everything else is refused: an americano pairs its later rounds *from the standings*, interclubes derives its very format from the entry count, and a groups+playoff already in its knockout phase has a bracket seeded from standings the entrant took no part in.

Two supporting changes make this safe:

- **Frozen group membership.** Group membership used to be re-derived on every read (`computeGroupMembership`), which breaks the moment one more entrant changes `computeGroupSizes` and reshuffles groups already being played. `freezeGroupMembership` now writes each competitor's group and slot into `competitors.data` (`{ groupNumber, groupPosition }`, JSONB — no migration) as the tournament starts, and both the engine and the views read it back.
  A tournament that was **already running** when this shipped carries none, so `resolveGroupMembership` re-derives it and only adopts it once it has been **verified against the matches**: every materialised fixture must sit in the group the derivation puts both its sides in, and (for an ordered lane, where the circle method depends on the order inside each group) each materialised round must come out with exactly the fixtures it already holds. If it checks out the repair is written down once, at registration time and *before* the entrant's row exists — deriving with them already counted in would place them wherever the split wanted rather than in the group the organizer picked. If it does not check out the category simply stays closed.
- **Unplayed early rounds stay editable.** `isMatchEditable`'s frontier rule guards *results*; a match that was never played has no consequence to protect, so it stays open even when a later round holds results. That is what makes the entrant's retroactive fixtures playable. Americanos are excluded (their later rounds are paired *from* the standings), and outside late registration it changes nothing — an ordered lane only materialises its next round once the frontier is fully resolved.

Covered by `tests/unit/late-registration.test.ts` (what is offered) and `tests/flows/late-registration.test.ts` (what taking a slot produces: preserved fixtures, complete round robins, playable matches, tournaments that still finish).

## Late removal (unregistering or moving inside a running tournament)

The mirror image of late registration, held to a **stricter** promise. Registration accepts an entrant wherever the structure has a hole shaped like them; removal is not allowed to **leave** one. The competitor has to come out *without a trace*, so that what remains is indistinguishable from the same category with one competitor fewer: no walkover awarded to a rival who never played, no fixture that changes round, rival or meaning, no dangling bracket slot. When that cannot be guaranteed the competitor simply stays where they are. `canRemoveCompetitor` (`utils/lateRemoval.ts`) returns the verdict **plus the sentence explaining it**, which is what the disabled button's tooltip shows and what the API throws — one explanation, whether the organizer is stopped before clicking or by the server.

Two conditions apply whatever the format: the competitor must not have **played** anything (a resolved match of theirs is a result that would be erased — a `VOID` fixture never happened and does not count), and their lane must not be left with fewer than two competitors.

- **Knockouts are closed, always.** Taking a competitor out of a first-round match turns it into a bye and hands their rival a walkover they never played; taking out somebody who *has* a bye leaves the match it feeds with a side that can never be filled, deadlocking the bracket. Both are traces. (Symmetrically: filling a bye is a hole-*filling* operation, and it has no inverse.)
- **A round-robin lane can let somebody go**, under the same reading of what a round means that governs arrivals:
  - **Ordered** — the circle method pads an odd lane with a null slot **at the end** of the id array, so the only competitor who can leave without re-pairing everybody else is the one sitting exactly there: the **last of an even lane**, whose departure turns it into the odd lane it would have been all along. Rather than assert that from the shape, it is checked against the fixtures themselves (`reproducesLaneWithout`): every materialised round must come out of the shrunken lane exactly as stored, minus the leaver's own matches. The round *count* is preserved for free — an even round robin of size *k* needs as many rounds as the odd one of size *k-1* it becomes.
  - **Unordered** — rounds are a display grouping and nothing is ever re-derived, so deleting the leaver's fixtures leaves every other one exactly where it is. Any member may leave.
- **Groups+playoff** additionally requires the category to still be in its **group phase**, on a membership that is provably the one being played, and that the group would send the **same number of qualifiers** to the knockout without them (`resolveGroupQualifiers` on the shrunken sizes must match) — a smaller group changing the cut-off is a change to the phase that follows.
- **Americano and interclubes are closed**, for the same reasons they accept no entrant.

**Moving a competitor** to another category is a departure followed by an arrival, and **both halves must be possible**: the origin must let them go *and* the destination must have a hole for them. Everything is resolved before the first write, so a move that cannot be completed does not half-happen. One extra check belongs only to moves: every slot on offer assumes a *brand-new* entrant, and an ordered league's lane is ordered by competitor **id**, so a fresh registration is by definition the highest id and lands on the rest slot — a competitor arriving from another category keeps their old id and may be inserted into the *middle* of the order instead. `slotAcceptsRelocatedCompetitor` verifies the arrival exactly the way the departure is verified, which in practice means the mover's id has to be above every id in the destination league. Restrictive, but honest: a league's fixture *is* its registration order, and there is no free way to insert somebody into the middle of one. Groups are immune (they append at an explicit `groupPosition`), as are unordered lanes and knockout byes.

On the engine side `detachLateCompetitor` deletes the leaver's fixtures, closes the positional gaps they left, drops their frozen group membership, and only *then* — once the caller's `leave` callback has actually taken them out of the category — advances it. The order is not optional: deleting the last pending fixture of a round completes it, and a competitor still counted as a member at that point would be handed back the very fixtures just deleted.

Covered by `tests/flows/late-removal.test.ts`: what stayed the same on both sides of a move, the absence of any trace of the leaver, the shrunken lane still playing to completion, and every refusal (played, not last, bracket phase, closed formats, destination league ordered later).

## Planning and publishing the schedule

Spanish vocabulary, so the UI reads consistently: everything the user sees says **"Programador"** (the organizer's tool) and **"Programación"** (what it produces, and what a player opens). The code keeps the English `planner` / `schedule` naming it was written with — `TournamentPlannerView`, `utils/planner.ts`, `/tournaments/:id/planner` — so a Spanish label and its identifier will not match word for word.

Where and when a match is played lives **on the match itself** (`matches.siteId` / `date` / `hour` / `courtNumber`, migration `013-matches-schedule`), so the schedule is visible everywhere a match is shown. Two screens read those four columns, from opposite ends:

- **The planner** (`/tournaments/:id/planner`, organizer-only) — a drag & drop board that places pending matches on a venue, a day, a court and a time (`POST /api/setMatchSchedule`, `POST /api/clearMatchSchedule`). Interclubes gets its own planner, because there the venue is not the organizer's to choose (a series is played at the home team's club), so the grid is laid out by *slot* rather than by court. Both export a branded PDF — a court x hour grid for the regular one, a four-column programme for interclubes — built by hand on top of the dependency-free writer in `app/utils/pdf.ts`.
- **The published schedule** (`/tournaments/:id/schedule`) — the same sheet, read-only, for the people who have to show up. Players reach it from a **"Ver programación"** button on the tournament page; the button only appears when there is something to show. It reproduces the exported PDF on screen (same palette, same bands, same two formats) and adds the one thing paper cannot: the reader's own matches are highlighted. It carries no logo, unlike the PDF — the app's own top bar is already showing it. A tournament spread across several venues gets **one sheet per venue**, each headed by its club and holding its own days, court columns and court names, because a venue is where a player physically goes and the other club's courts are of no use to them. The day window, though, is derived once for the whole tournament, so every sheet covers the same block of play and a venue idle during it simply has no sheet.

Both derive their match list from the same pure builder, `utils/planner.ts` (`buildPlannerEntries`), so a card, a PDF cell and a published row describe a match identically.
