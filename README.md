...
- **Mercado Pago** split payments (marketplace) for paid tournament registrations
...
app/
  ...
  services/               Shared BE services (organizations, mercadopago, mercadopago-oauth, pwa)
...
app/(protected)/
  ...
  (account)/               My-account module (profile, Mercado Pago connection)
...
- **Method**: always `POST`, with a JSON body for the parameters — except the Mercado Pago webhook and the `processTournaments` Vercel Cron endpoint, which are plain `GET`/`POST` handlers outside this contract (see "Registration payments" and "Domain notes").
...
- Mercado Pago notifies `POST /api/processTournamentPaymentState?ref=<paymentId>` (a public endpoint outside the standard `ApiResponse` contract, with its own `x-signature` verification when `MP_WEBHOOK_SECRET` is set). The handler fetches the real payment, and on `approved` it registers the competitor (re-validating capacity/duplicates inside a transaction) and marks the payment `APPROVED`. If the place can no longer be honoured it refunds the payment (`REFUNDED`). The webhook is idempotent.
