...
describe('score — THREE_SETS validation', () => {
  ...
  it('accepts straight sets and a decided third set', () => {
    expect(isValidScore({ sets: [{ home: 6, away: 4 }, { home: 6, away: 3 }] }, f)).toBe(true)
    expect(isValidScore({ sets: [{ home: 6, away: 4 }, { home: 4, away: 6 }, { home: 6, away: 3 }] }, f)).toBe(true)
    expect(isValidScore({ sets: [{ home: 7, away: 5 }, { home: 7, away: 6 }] }, f)).toBe(true)
  })

  it('rejects a 1-1 set split with no decider (tie)', () => {
    expect(isValidScore({ sets: [{ home: 6, away: 4 }, { home: 4, away: 6 }] }, f)).toBe(false)
  })

  it('rejects irregular set scores', () => {
    expect(isValidScore({ sets: [{ home: 8, away: 6 }, { home: 6, away: 3 }] }, f)).toBe(false)
    expect(isValidScore({ sets: [{ home: 6, away: 5 }, { home: 6, away: 3 }] }, f)).toBe(false)
  })

  it('rejects a third set when the first two are not split', () => {
    // Home already won both → a third set is illegal.
    expect(isValidScore({ sets: [{ home: 6, away: 4 }, { home: 6, away: 3 }, { home: 6, away: 2 }] }, f)).toBe(false)
  })

  it('rejects more than three sets', () => {
    expect(
      isValidScore(
        { sets: [{ home: 6, away: 4 }, { home: 4, away: 6 }, { home: 6, away: 3 }, { home: 6, away: 0 }] },
        f
      )
    ).toBe(false)
  })
...
describe('score — TWO_SETS_SUPER_TIEBREAK validation', () => {
  ...
  it('accepts a valid super tiebreak as the third set', () => {
    expect(isValidScore({ sets: [{ home: 6, away: 4 }, { home: 4, away: 6 }, { home: 10, away: 8 }] }, f)).toBe(true)
    expect(isValidScore({ sets: [{ home: 6, away: 4 }, { home: 4, away: 6 }, { home: 11, away: 9 }] }, f)).toBe(true)
    expect(isValidScore({ sets: [{ home: 6, away: 4 }, { home: 4, away: 6 }, { home: 13, away: 11 }] }, f)).toBe(true)
  })

  it('rejects an invalid super tiebreak (no win-by-two beyond 10)', () => {
    expect(isValidScore({ sets: [{ home: 6, away: 4 }, { home: 4, away: 6 }, { home: 10, away: 9 }] }, f)).toBe(false)
    expect(isValidScore({ sets: [{ home: 6, away: 4 }, { home: 4, away: 6 }, { home: 12, away: 9 }] }, f)).toBe(false)
  })
})
...
describe('score — winner / counting helpers', () => {
  it('determines the set winner', () => {
    const score = { sets: [{ home: 6, away: 4 }, { home: 3, away: 6 }, { home: 6, away: 2 }] }
  ...
  it('counts games across sets', () => {
    const score = { sets: [{ home: 6, away: 4 }, { home: 3, away: 6 }] }
  ...
  it('returns null winner for an undecided set score', () => {
    expect(getScoreWinner({ sets: [{ home: 6, away: 4 }, { home: 4, away: 6 }] }, ScoreFormat.THREE_SETS)).toBeNull()
  })
})
