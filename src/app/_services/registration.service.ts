...
/** Server-side registration domain logic (used only by API route handlers). */
...
export async function searchUsers(userId: number, query: string): Promise<UserDto[]> {
...
/** Registers a user (optionally with a partner) into a tournament. */
...
/** Removes the user registration while the tournament is in stand_by. */
export async function leaveTournament(userId: number, tournamentId: number): Promise<ServiceResult> {
  const tournament: Tournament | null = await Tournament.find(tournamentId)
